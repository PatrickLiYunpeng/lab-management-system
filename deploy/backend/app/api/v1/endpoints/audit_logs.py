"""
审计日志API端点 - Audit Log API Endpoints

本模块提供审计日志查询接口（只读），用于追踪系统中的所有操作记录。
审计日志记录用户的操作行为、数据变更历史，支持合规审计和问题排查。

API端点列表:
- GET /audit-logs: 分页获取审计日志列表
    - 支持按用户、操作类型、实体类型、实体ID筛选
    - 支持按实验室、站点筛选
    - 支持按日期范围筛选
    - 支持关键词搜索
...
- entity_type: 实体类型（work_order/task/personnel等）
- entity_id: 实体ID
- old_values: 变更前的值（JSON格式）
- new_values: 变更后的值（JSON格式）
- ip_address: 操作者IP地址
- request_path: 请求路径
- created_at: 操作时间
