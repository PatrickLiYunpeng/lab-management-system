...
from app.models.equipment_category import EquipmentNameModel
...
def list_equipment(
    ...
    category: Optional[List[str]] = Query(None, description="Filter by equipment categories (multiple values supported)"),
    ...
    Args:
        category: Filter by equipment categories. Supports multiple values.
            Example: ?category=thermal&category=mechanical
    """
    query = db.query(Equipment).options(
        joinedload(Equipment.equipment_name)  # 预加载 equipment_name 关系（包含 is_critical）
    )
    ...
    if category:
        # 支持多个 category 值筛选
        try:
            category_enums = [EquipmentCategory(c) for c in category]
            query = query.filter(Equipment.category.in_(category_enums))
        except ValueError:
            # 如果 category 值无效，忽略该筛选条件
            pass
    if laboratory_id:
...
def get_equipment(
    ...
    equipment = db.query(Equipment).options(
        joinedload(Equipment.equipment_name)  # 预加载 equipment_name（包含 is_critical）
...
def create_equipment_schedule(
    ...
    # Invalidate gantt cache when schedule is created
...
def delete_equipment_schedule(
    ...
    # Invalidate gantt cache when schedule is deleted
    gantt_cache.invalidate_pattern("gantt:")
...
def get_gantt_data(
    ...
    is_critical: Optional[bool] = Query(None, description="Filter by critical equipment (is_critical flag in equipment_names)"),
    equipment_ids: Optional[List[int]] = Query(None, description="Filter by specific equipment IDs (overrides other filters)"),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_active_user)
):
    """
    ...
    Cache is keyed by query parameters.
    
    Args:
        is_critical: If True, returns only critical equipment; if False, returns only non-critical.
    """
    from sqlalchemy.orm import joinedload
    from app.core.config import settings
    
    # Generate cache key from query parameters
    cache_key = f"gantt:{start_date.isoformat()}:{end_date.isoformat()}:{laboratory_id}:{site_id}:{equipment_type}:{category}:{is_critical}:{','.join(map(str, equipment_ids or []))}"
    
    # Try to get from cache (skip in testing mode)
    if not settings.TESTING:
        hit, cached_result = gantt_cache.get(cache_key)
        if hit:
    ...
    # If equipment_ids provided, use it directly (overrides other filters)
    if equipment_ids:
        eq_query = eq_query.filter(Equipment.id.in_(equipment_ids))
    else:
        # Apply other filters only if equipment_ids not provided
        if laboratory_id:
        ...
        # Filter by is_critical via equipment_names table
        if is_critical is not None:
            eq_query = eq_query.join(
                EquipmentNameModel,
                Equipment.equipment_name_id == EquipmentNameModel.id
            ).filter(EquipmentNameModel.is_critical == is_critical)
    ...
    # Store in cache (skip in testing mode)
    if not settings.TESTING:
        gantt_cache.set(cache_key, {**result, "cached": True})
    
    return result


# Cache statistics endpoint (for monitoring)
@router.get("/schedules/gantt/cache-stats")
def get_gantt_cache_stats(
    current_user: User = Depends(require_manager_or_above)
):
    """
    Get Gantt chart cache statistics.
    
    Requires manager or above role.
    Returns cache hit rate, size, and other metrics.
    """
    return {
        "cache_name": "gantt_cache",
        "ttl_seconds": gantt_cache.default_ttl,
        **gantt_cache.stats
    }
