"""
报表生成API端点 - Report Generation API Endpoints

本模块提供PDF报表和其他格式导出的API接口。
支持工单报表、设备报表、人员报表等多种报表类型的生成和下载。

API端点列表:
- GET /reports/work-orders/pdf: 导出工单列表PDF报表
    - 支持按实验室、工单类型、状态、客户筛选
    - 支持按日期范围筛选
- GET /reports/equipment/pdf: 导出设备列表PDF报表
- GET /reports/personnel/pdf: 导出人员列表PDF报表

报表内容:
- 工单报表：工单编号、客户、状态、创建时间、完成时间等
- 设备报表：设备编号、名称、状态、所属实验室等
- 人员报表：人员信息、技能、班次分配等
...
技术说明:
- 使用StreamingResponse返回PDF文件
- 支持大数据量分页导出
- PDF生成使用PDFReportService服务
"""
