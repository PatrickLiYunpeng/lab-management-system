"""
仪表盘API端点 - Dashboard API Endpoints

本模块提供仪表盘统计数据和关键绩效指标(KPI)的API接口。
支持实时数据展示、设备利用率、人员效率、任务完成情况等多维度分析。

API端点列表:

综合概览:
- GET /dashboard/summary: 获取仪表盘综合摘要
    - 人员统计：在岗/离岗/总数
    - 设备统计：运行中/空闲/维护中
    - 工单统计：待处理/进行中/已完成
    - 物料统计：在库/使用中/超期
...
设备分析:
- GET /dashboard/equipment: 获取设备仪表盘数据
    - 按类别统计设备数量和状态
    - 设备利用率分析
    - 设备故障率统计

任务分析:
- GET /dashboard/tasks: 获取任务完成统计
    - 任务完成率
    - 平均完成时间
    - 超期任务统计

SLA分析:
- GET /dashboard/sla: 获取SLA达成情况
    - SLA达成率
    - 按客户/实验室统计

周期时间分析:
- GET /dashboard/cycle-time: 获取周期时间统计
    - 平均周期时间
    - 趋势分析

工作负载:
- GET /dashboard/workload: 获取工作负载分析
    - 人员工作负载
    - 设备负载分布

筛选条件:
- laboratory_id: 按实验室筛选
- site_id: 按站点筛选
- date_range: 按日期范围筛选
