"""
客户SLA与测试来源类别管理API端点 - Client SLA & Testing Source Category API Endpoints

本模块提供客户服务等级协议(SLA)和测试来源类别的管理API。
SLA定义了不同客户、不同测试类型的响应时间承诺，是周期时间计算的基础。

API端点列表:

客户SLA管理:
- GET /clients/slas: 分页获取SLA配置列表
    - 支持按客户、实验室、方法类型、来源类别筛选
- GET /clients/slas/{sla_id}: 获取单个SLA配置详情
- POST /clients/slas: 创建SLA配置（Manager及以上角色）
- PUT /clients/slas/{sla_id}: 更新SLA配置（Manager及以上角色）
- DELETE /clients/slas/{sla_id}: 删除SLA配置（Manager及以上角色）

测试来源类别管理:
- GET /clients/source-categories: 获取测试来源类别列表
- GET /clients/source-categories/{id}: 获取单个来源类别详情
- POST /clients/source-categories: 创建来源类别（Manager及以上角色）
- PUT /clients/source-categories/{id}: 更新来源类别（Manager及以上角色）
- DELETE /clients/source-categories/{id}: 删除来源类别（Manager及以上角色）

SLA配置字段:
- client_id: 客户ID
- laboratory_id: 实验室ID
- method_type: 方法类型（FA分析/Reliability测试）
- source_category_id: 测试来源类别ID
- target_hours: 目标完成时间（小时）
- warning_threshold_percent: 预警阈值百分比
- is_active: 是否启用

测试来源类别:
- 用于区分不同来源的测试请求（如：生产线、客诉、研发等）
- 不同来源可能有不同的SLA要求
