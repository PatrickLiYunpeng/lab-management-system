...
def get_equipment_names_by_category(
    ...
    names = query.order_by(EquipmentNameModel.name).all()
    return names
...
@router.get("/equipment-names/{name_id}/equipment")
def get_equipment_by_name(
    name_id: int,
    site_id: int = Query(..., description="站点ID（必填）"),
    is_active: bool = Query(True, description="是否仅返回激活设备"),
    db: Session = Depends(get_db)
):
    """
    获取指定设备名下的所有设备实例
    
    用于关键设备调度场景：当选择关键设备时，获取该站点下所有同名设备。
    
    - **name_id**: 设备名ID
    - **site_id**: 站点ID（必填，限制在当前站点）
    - **is_active**: 是否仅返回激活设备（默认True）
    """
    # 验证设备名存在
    equipment_name = db.query(EquipmentNameModel).filter(
        EquipmentNameModel.id == name_id
    ).first()
    ...
    return {
        "equipment_name_id": name_id,
        "equipment_name": equipment_name.name,
        "is_critical": equipment_name.is_critical,
        "site_id": site_id,
        "items": [
            {
                "id": e.id,
                "name": e.name,
                "code": e.code,
                "equipment_type": e.equipment_type.value if e.equipment_type else None,
