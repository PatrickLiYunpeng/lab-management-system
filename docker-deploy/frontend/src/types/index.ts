/**
 * 实验室管理系统 - TypeScript类型定义
 * Laboratory Management System - TypeScript Type Definitions
 * 
 * 本模块定义系统所有的TypeScript类型和接口，与后端API响应数据结构对应。
 * 类型定义分为以下几个主要部分：
 * 
 * 用户与认证 (User & Auth):
 * - UserRole: 用户角色枚举 (admin/manager/engineer/technician/viewer)
 * - User: 用户信息接口
 ...
 * - Site: 站点信息
 * - Laboratory: 实验室信息
 * - LaboratoryType: 实验室类型 (fa/reliability)
 * 
 * 人员管理 (Personnel):
 * - Personnel: 人员信息
 * - PersonnelStatus: 人员状态 (available/busy/on_leave/borrowed)
 * - PersonnelSkill: 人员技能
