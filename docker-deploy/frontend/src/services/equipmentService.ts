 ...
 * 
 * 功能:
 * - 设备CRUD：获取列表、详情、创建、更新、删除
 * - 设备类别管理：类别和设备名称的增删改查
 * - 设备排程：获取甘特图数据、创建/更新排程
 * - 容量管理：查询设备可用容量
 * 
 * 方法:
 * - getEquipment(): 获取设备列表（分页、筛选）
 * - getEquipmentById(): 获取设备详情
 * - createEquipment(): 创建新设备
 * - updateEquipment(): 更新设备信息
 * - getEquipmentCategories(): 获取设备类别列表
 * - getEquipmentNames(): 获取设备名称列表
 * - getGanttData(): 获取设备排程甘特图数据
 * - getEquipmentCapacity(): 获取设备容量信息
 */
import api from './api';
import type {
  Equipment,
