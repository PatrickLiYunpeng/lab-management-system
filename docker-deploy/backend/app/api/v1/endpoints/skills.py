"""
技能管理API端点 - Skills Management API Endpoints

本模块提供技能定义和人员技能分配的API接口。
技能系统用于追踪人员的能力资质，支持任务分配时的技能匹配。

API端点列表:

技能定义:
- GET /skills: 分页获取技能列表，支持按类别、实验室类型筛选
- GET /skills/{skill_id}: 获取单个技能详情
- POST /skills: 创建新技能（Manager及以上角色）
- PUT /skills/{skill_id}: 更新技能信息（Manager及以上角色）
- DELETE /skills/{skill_id}: 删除技能（Manager及以上角色）

人员技能分配:
- GET /skills/personnel/{personnel_id}: 获取指定人员的技能列表
- POST /skills/personnel-skills: 为人员分配技能
- PUT /skills/personnel-skills/{id}: 更新人员技能（熟练度、证书等）
- DELETE /skills/personnel-skills/{id}: 移除人员技能

技能分类:
- ANALYSIS: 分析技能（FA实验室）
- TESTING: 测试技能（Reliability实验室）
- EQUIPMENT: 设备操作技能
- GENERAL: 通用技能
